const skins = require('./configs/skins.json').Skins;
const spawnPoints = require('./configs/spawn_points.json').SpawnPoints;
...
mp.events.add('clientData', function() {
    ...
    */
	const args = JSON.parse(arguments[1]);
	...
	case 'flip':
		if (player.vehicle) {
			const rotation = player.vehicle.rotation;
			rotation.y = 0;
	...
	case 'server_color':
		if (player.vehicle) {
			if (args[1] == 'color') {
				const colorPrimary = JSON.parse(args[2]);
				const colorSecondary = JSON.parse(args[3]);
